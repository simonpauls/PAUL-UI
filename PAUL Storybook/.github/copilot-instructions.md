# Simple Design System (PAUL) Copilot Instructions
...
PAUL is a production-ready design system featuring:
...
- [Live Storybook](https://figma.github.io/paul/storybook) - Component documentation
...
3. ✅ **Use PAUL components only** - never import from `@react-aria`, `@react-stately`, etc.
...
When importing PAUL components, use aliases to ensure correct paths. These are described in the `vite.config.ts` file under `resolve.alias`:
...
## Figma to PAUL Translation Guidelines

When implementing designs from Figma, follow these guidelines to ensure proper translation to the PAUL component library.
...
- Colors: Use `var(--paul-color-*)` variables (e.g., `var(--paul-color-text-default-default)`)
- Spacing: Use `var(--paul-size-space-*)` variables (e.g., `var(--paul-size-space-400)`)
  - **important**: These variables can map to a prop value numerically. For example, `<Section padding="400">` corresponds to `padding: var(--paul-size-space-400)` in a Figma response.
- Typography: Use `var(--paul-typography-*)` or `var(--paul-font-*)` variables
- Border radius: Use `var(--paul-size-radius-*)` variables
- Shadows: Use `var(--paul-effects-shadows-*)` variables
...
.custom-element {
  ...
  color: var(--paul-color-text-default-default);
  padding: var(--paul-size-space-400);
  border-radius: var(--paul-size-radius-200);
}
```
...
All UI components are in [src/ui](../src/ui). **Never create new components - always use existing PAUL components.**
...
2. **Identify PAUL components**: Map Figma components to existing PAUL primitives, compositions, and layouts
...
**Fix**: Use CSS variables via class names: `<div className="example">` and `.example { color: var(--paul-color-text-default-default); padding: var(--paul-size-space-400); }`
...
PAUL uses Figma Code Connect for seamless design-to-code integration. Here's how it works:

- **Design Tokens**: Figma styles are synced as CSS custom properties (e.g., `--paul-color-primary`)
- **Components**: Figma components are mapped to PAUL components with equivalent props and variants
...
1. Use the **"PAUL - Design Tokens"** and **"PAUL - Components"** styles in Figma
2. Organize components in Figma using the same structure as PAUL (primitives, compositions, etc.)
3. Publish changes in Figma, then run `npm run script:tokens` and `npm run script:icons` to sync

### Known Limitations

- Not all Figma features have 1:1 parity with PAUL components (e.g., complex interactions)
- Some manual adjustment may be needed after syncing, especially for layout and spacing
- Ensure Figma components are named and organized according to PAUL conventions for best results
