...
import { AnchorOrButton, type AnchorOrButtonProps, getBrandingConfig } from "utils";
import "./logo.css";

export type LogoProps = AnchorOrButtonProps;
export function Logo({ className, ...props }: LogoProps) {
  const classNames = clsx(className, "logo");
  const { logoPath, brandName } = getBrandingConfig();

  return (
    <AnchorOrButton className={classNames} {...props} aria-label={`${brandName} logo`}>
      {logoPath && logoPath !== "" ? (
        <img src={logoPath} alt={`${brandName} logo`} style={{ height: "100%", width: "auto" }} />
      ) : (
        <svg
          width="172"
          height="247"
